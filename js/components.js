...
const GYM_DATA = [
  { id: "gym1", city: "ニビシティ",     leader: "タケシ", type: "いわ",    lv: "Lv.12–14" },
  { id: "gym2", city: "ハナダシティ",   leader: "カスミ", type: "みず",    lv: "Lv.18–21" },
  { id: "gym3", city: "クチバシティ",   leader: "マチス", type: "でんき",  lv: "Lv.18–24" },
  { id: "gym4", city: "タマムシシティ", leader: "エリカ", type: "くさ",    lv: "Lv.29–33" },
  { id: "gym5", city: "セキチクシティ", leader: "キョウ", type: "どく",    lv: "Lv.37–43" },
  { id: "gym6", city: "ヤマブキシティ", leader: "ナツメ", type: "エスパー",lv: "Lv.38–43" },
  { id: "gym7", city: "グレンじま",     leader: "カツラ", type: "ほのお",  lv: "Lv.42–47" },
  { id: "gym8", city: "トキワシティ",   leader: "サカキ", type: "じめん",  lv: "Lv.45–50" },
];

const ELITE4_DATA = [
  { id: "e4_1",     name: "カンナ",   type: "こおり",   lv: "Lv.52–56" },
  { id: "e4_2",     name: "シバ",     type: "かくとう", lv: "Lv.54–58" },
  { id: "e4_3",     name: "キクコ",   type: "ゴースト", lv: "Lv.54–58" },
  { id: "e4_4",     name: "ワタル",   type: "りゅう",   lv: "Lv.56–62" },
  { id: "champion", name: "グリーン", type: null,        lv: "Lv.59–65" },
];

function StoryProgressPanel({ color, checkedStory, onToggle }) {
  const [open, setOpen] = useState(true);

  const allIds  = [...GYM_DATA, ...ELITE4_DATA].map(m => m.id);
  const total   = allIds.length;
  const done    = allIds.filter(id => checkedStory[id]).length;
  const gymDone = GYM_DATA.filter(g => checkedStory[g.id]).length;

  return (
    <Panel title="🏅 ストーリー進捗" open={open} onToggle={() => setOpen(o => !o)} color={color}>
      {/* 進捗バー */}
      <div style={{ marginBottom: "12px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "10px", marginBottom: "4px" }}>
          <span style={{ color: "#666" }}>バッジ {gymDone}/8</span>
          <span style={{ color: done >= total ? "#7fff7f" : "#555" }}>{done}/{total}</span>
        </div>
        <div className="bar-bg" style={{ height: "6px" }}>
          <div className="bar-fill" style={{
            height: "100%", borderRadius: "4px",
            width: `${total > 0 ? (done / total) * 100 : 0}%`,
            background: done >= total ? "#7fff7f" : `linear-gradient(90deg, ${color}88, ${color})`,
          }} />
        </div>
      </div>

      {/* ジム */}
      <div style={{ fontSize: "10px", color: "#444", marginBottom: "6px", letterSpacing: "1px" }}>ジムリーダー</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5px", marginBottom: "14px" }}>
        {GYM_DATA.map((gym, i) => {
          const tc      = TYPE_COLORS[gym.type] || "#555";
          const cleared = !!checkedStory[gym.id];
          return (
            <div key={gym.id} onClick={() => onToggle(gym.id)} style={{
              background: cleared ? tc + "1a" : "#0d1525",
              border: `1px solid ${cleared ? tc + "99" : "#252535"}`,
              borderRadius: "7px", padding: "7px 9px", cursor: "pointer",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: "5px", marginBottom: "3px" }}>
                <span style={{ fontSize: "8px", color: "#333" }}>#{i + 1}</span>
                <span style={{ fontSize: "8px", background: tc + "22", color: tc, border: `1px solid ${tc}44`, borderRadius: "3px", padding: "0 4px" }}>{gym.type}</span>
                {cleared && <span style={{ fontSize: "9px", color: tc, marginLeft: "auto" }}>✓</span>}
              </div>
              <div style={{ fontSize: "11px", color: cleared ? tc : "#bbb", fontWeight: cleared ? "bold" : "normal" }}>{gym.leader}</div>
              <div style={{ fontSize: "9px", color: "#444", marginTop: "1px" }}>{gym.city}</div>
              <div style={{ fontSize: "8px", color: "#333", marginTop: "1px" }}>{gym.lv}</div>
            </div>
          );
        })}
      </div>
      ...
      {/* 四天王・チャンピオン */}
      <div style={{ fontSize: "10px", color: "#444", marginBottom: "6px", letterSpacing: "1px" }}>四天王 ＆ チャンピオン</div>
      <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
        {ELITE4_DATA.map((e, i) => {
          const tc      = e.type ? (TYPE_COLORS[e.type] || "#888") : color;
          const cleared = !!checkedStory[e.id];
          return (
            <div key={e.id} onClick={() => onToggle(e.id)} style={{
              display: "flex", alignItems: "center", gap: "9px",
              background: cleared ? tc + "1a" : "#0d1525",
              border: `1px solid ${cleared ? tc + "99" : "#252535"}`,
              borderRadius: "7px", padding: "8px 10px", cursor: "pointer",
            }}>
              <span style={{ fontSize: "10px", color: cleared ? tc : "#333", minWidth: "12px" }}>{cleared ? "✓" : String(i + 1)}</span>
              <span style={{ flex: 1, fontSize: "12px", color: cleared ? tc : "#aaa", fontWeight: cleared ? "bold" : "normal" }}>
                {e.id === "champion" ? `👑 ${e.name}` : e.name}
              </span>
              {e.type && (
                <span style={{ fontSize: "9px", background: tc + "22", color: tc, border: `1px solid ${tc}44`, borderRadius: "3px", padding: "1px 5px" }}>{e.type}</span>
              )}
              <span style={{ fontSize: "9px", color: "#333" }}>{e.lv}</span>
            </div>
          );
        })}
      </div>
